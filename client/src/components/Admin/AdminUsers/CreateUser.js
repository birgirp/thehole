...
class CreateUser extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.editingUser) {
      this.setState({
        editingUser: true,
        editingUserId: this.props.userId,
        loading: true,
      })
      this.setState({})
      axios
  ...
  handleChange = (e) => {
    ...
    switch (name) {
      case 'fullName':
        ...
        break
      case 'handicap':
        formErrors.handicap = isNaN(value) ? 'Handicap must be a number' : ''
        break
      case 'email':
    ...
    if (this.props.editingProfile) {
      ...
      axios
        ...
        .then((response) => {
          ...
          this.setState({ loading: false }, () => this.props.cancel())
        })
        .catch((error) => {
          console.log(error)
        })
          ...
          <Form>
            <Form.Group widths='equal'>
              <Grid style={{ width: '95%', marginLeft: '5%' }}>
                ...
                <Grid.Row columns={1}>
                  <Grid.Column>
                    {this.props.editingProfile === false && (
                      <Form.Field
                        control={Checkbox}
                        label='Is admin'
