...
class Lonessome extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .post("/api/getteammembers", { tourId: this.props.tourId })
      .then(res => {
        let teams = res.data;
        ...
        teamA = res.data.filter(team => {
          return team.team_id === this.props.idA;
        });
        teamB = res.data.filter(team => {
          return team.team_id === this.props.idB;
        });

        return axios.post("/api/getlonesomegame", {
        ...
        if (res2.data) {
          ...
          for (let i = 0; i < teamA.length; i++) {
            ...
            if (scores.some(p => p.player_id === playerA.player_id)) {
              playerA.score = parseInt(
                scores.filter(p => p.player_id === playerA.player_id)[0].score
              );
            } else {
              playerA.score = 0;
            }
            ...
            if (scores.some(p => p.player_id === playerB.player_id)) {
              playerB.score = parseInt(
                scores.filter(p => p.player_id === playerB.player_id)[0].score
              );
            } else {
              playerB.score = 0;
            }
  ...
  handleSubmit = () => {
    ...
    if (this.state.firstTime) {
      ...
      axios
        .post("/api/deletelonesomescores", { gameId: this.props.game.id })
        .then(res => {
          this.updateLonesomeScores();
        })
        .catch(err => {
          ...
          <Table celled>
            ...
            <Table.Body>
              {teamA.map((player, index) => {
                return (
                  <Table.Row key={index}>
                    <Table.Cell>{player.full_name}</Table.Cell>
                    <Table.Cell>{player.handicap}</Table.Cell>
                    <Table.Cell>
                      <Input
                        index={index}
                        name="Score"
                    ...
                    <Table.Cell>{teamB[index].full_name}</Table.Cell>
                    <Table.Cell>{teamB[index].handicap}</Table.Cell>
                    <Table.Cell>
                      <Input
                        index={index}
                        name="Score"
            ...
            <Table.Footer>
              <Table.Row>
                ...
                <Table.HeaderCell></Table.HeaderCell>
                <Table.HeaderCell>
                  <b>{this.state.sumScoresA}</b>
                </Table.HeaderCell>
                <Table.HeaderCell></Table.HeaderCell>
                <Table.HeaderCell></Table.HeaderCell>
                <Table.HeaderCell>
                  <b>{this.state.sumScoresB}</b>
                </Table.HeaderCell>
              </Table.Row>
            </Table.Footer>
          </Table>
